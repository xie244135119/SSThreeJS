...
/*
 * Author  Kayson.Wan
 * Date  2022-09-15 15:23:33
 * LastEditors  xie244135119
 * LastEditTime  2022-10-19 16:44:38
 * Description
 */
...
const helpVec31 = new THREE.Vector3();
const helpVec32 = new THREE.Vector3();
const helpVec33 = new THREE.Vector3();
...
function generatePathVertexData(pathPointList, options, generateUv2 = false) {
  ...
  const arrow = options.arrow !== undefined ? options.arrow : false;
  ...
  if (progressDistance > 0) {
    ...
    const { point } = pathPointList.array;
    lastPoint = point;
  }
...
function generateTubeVertexData(pathPointList, options, generateUv2 = false) {
  ...
  function addVertices(pathPoint, radius, radialSegments) {
    ...
    for (let r = 0; r <= radialSegments; r++) {
      ...
      normalDir
        .copy(pathPoint.up)
        .applyAxisAngle(pathPoint.dir, startRad + (Math.PI * 2 * _r) / radialSegments)
        .normalize();
...
function _getCornerBezierCurve(last, current, next, cornerRadius, firstCorner, out) {
  const lastDir = helpVec31.subVectors(current, last);
  const nextDir = helpVec32.subVectors(next, current);
...
class PathPointList {
   ...
   */
  set(aPoints, cornerRadius = 0.1, cornerSplit = 10, up = null, close = false) {
    const points = aPoints.slice(0);
    ...
    for (let i = 0, l = points.length; i < l; i++) {
      if (i === 0) {
        ...
        if (close) {
          ...
          // Fix start point
          const { dist } = this.array[0]; // should not copy dist
  ...
  _end(current) {
    ...
    const vec = helpVec31.crossVectors(lastPoint.dir, point.dir);
  ...
  _sharpCorner(current, next, up, dirType = 0, sharp = false) {
    ...
    const lastDir = helpVec31.subVectors(current, lastPoint.pos);
    const nextDir = helpVec32.subVectors(next, current);
    ...
    if (up) {
      ...
      const vec = helpVec33.crossVectors(lastPoint.dir, point.dir);
      if (vec.length() > Number.EPSILON) {
        vec.normalize();
        const theta = Math.acos(Math.min(Math.max(lastPoint.dir.dot(point.dir), -1), 1)); // clamp for floating pt errors
        point.up.applyMatrix4(helpMat4.makeRotationAxis(vec, theta));
...
class PathGeometry extends THREE.BufferGeometry {
  ...
  _initByMaxVertex(maxVertex, generateUv2) {
    this.setAttribute(
      'position',
      new THREE.BufferAttribute(new Float32Array(maxVertex * 3), 3).setUsage(THREE.DynamicDrawUsage)
    );
    this.setAttribute(
      'normal',
      new THREE.BufferAttribute(new Float32Array(maxVertex * 3), 3).setUsage(THREE.DynamicDrawUsage)
    ...
    this.setAttribute(
      'uv',
      new THREE.BufferAttribute(new Float32Array(maxVertex * 2), 2).setUsage(THREE.DynamicDrawUsage)
    );
    if (generateUv2) {
      this.setAttribute(
        'uv2',
  ...
  _initByData(pathPointList, options, usage, generateUv2) {
    ...
    if (vertexData && vertexData.count !== 0) {
      this.setAttribute(
        'position',
        new THREE.BufferAttribute(new Float32Array(vertexData.position), 3).setUsage(
          usage || THREE.StaticDrawUsage
        )
      );
      this.setAttribute(
        'normal',
        new THREE.BufferAttribute(new Float32Array(vertexData.normal), 3).setUsage(
          usage || THREE.StaticDrawUsage
        )
      );
      this.setAttribute(
      ...
      if (generateUv2) {
        this.setAttribute(
          'uv2',
          new THREE.BufferAttribute(new Float32Array(vertexData.uv2), 2).setUsage(
            usage || THREE.StaticDrawUsage
          )
        );
      }
