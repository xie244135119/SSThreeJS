...
const baseConfig = require('../webpack/webpack.config.library');
...
const combileHandler = (_, stats) => {
  ...
  if (stats.hasErrors()) {
    ...
    console.log(
      chalk.bgRedBright(' ERROR ') +
        chalk.redBright(
          ` ${LOG_PREFIX} failed to build with ${statJsonObj.errors?.length} errors`
        ) +
  ...
  console.log(
    chalk.bgGreenBright(' DONE ') +
      chalk.greenBright(` ${LOG_PREFIX} build successfully in ${statJsonObj.time / 1000}s at `) +
      (statJsonObj.warnings?.length > 0
        ? chalk.redBright(`${statJsonObj.warnings?.length} types of warnings)`)
        : '')
  );
