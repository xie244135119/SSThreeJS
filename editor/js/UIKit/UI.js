...
class UIElement {
  ...
  add() {
    for (let i = 0; i < arguments.length; i++) {
      ...
      // console.log(' 添加子组件 ', argument, argument instanceof UIElement);
  ...
  addDom() {
    for (let i = 0; i < arguments.length; i++) {
      ...
      if (argument instanceof HTMLElement) {
        this.dom.appendChild(argument);
      } else {
        console.trace();
        console.error('HTMLElement:', argument, 'is not an instance of HTMLElement.');
      }
    }
...
class UITabbedPanel extends UIDiv {
  ...
  select(id) {
    ...
    if (this.selected && this.selected.length) {
      tab = this.tabs.find((item) => item.dom.id === scope.selected);
      panel = this.panels.find((item) => item.dom.id === scope.selected);
    ...
    tab = this.tabs.find((item) => item.dom.id === id);
    panel = this.panels.find((item) => item.dom.id === id);
  ...
  /**
   * 添加tab标签
   * @param {string} id id项目
   * @param {string} label tab标题
   * @param {} items
   */
